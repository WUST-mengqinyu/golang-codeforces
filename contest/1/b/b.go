...
func main() {
	...
	for kase := 1; kase <= T; kase++ {
		...
		flag := false
		for i, x := range s {
			if !flag && unicode.IsDigit(x) {
				flag = true
				tag = i
			} else if flag && !unicode.IsDigit(x) {
				t = 1
				tag = i
				break
			}
		...
		if t == 0 {
			C := int64(0)
			for i := 0; i < tag; i++ {
				C = C * 26 + int64(s[i] - 'A' + 1)
			}
			putchar('R')
			printRuneSlice([]rune(s[tag:]), false)
			putchar('C')
			printlnInt64s(C)
		} else {
			C := int64(0)
			flag = false
			for i := tag + 1; i < len(s); i++ {
				C = C * 10 + int64(s[i] - '0')
			}
			var res []rune
			for C >= 26 {
				if C % 26 == 0 {
					...
					res = append(res, rune('A' + C % 26 - 1))
					C /= 26
				}
			}
			if C > 0 {
				res = append(res, rune('A' + C - 1))
			}
			reverse(&res, 0, len(res))
			printRuneSlice(res, false)
			printRuneSlice([]rune(s[1:tag]), true)
		}
 	}
	close(channelType)
	oWg.Wait()
